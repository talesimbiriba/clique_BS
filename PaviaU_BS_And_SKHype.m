...
[rmse_skp, std_skp] = RMSEAndSTDForMatrix_bigData(r_skp,y);
...
%[rmse_kkm, std_kkm] = RMSEAndSTDForMatrix_bigData(r_kkm,y);

[rmse, stdd] = RMSEAndSTDForMatrix_bigData(a_kkm, a_skp);
...
for m=ms    
    ...
    %[rmse_clique, std_clique] = RMSEAndSTDForMatrix_bigData(r_clique,y);
    ...
    %[rmse_greedy, std_greedy] = RMSEAndSTDForMatrix_bigData(r_greedy,y);
...
%     printf('GKKM & %.4f $\\pm$ %.4f & %.2f & %d %.4f\n', rmse_kkm, std_kkm, Nb_kkm, mu_kkm);
    ...
    [rmse, stdd] = RMSEAndSTDForMatrix_bigData(a_clique, a_skp);
    fprintf('CCBS & %.4f $\\pm$ %.4f & %.4f & %d & %.4f\\\\ \n', rmse, stdd, cliqueCBSTime(i), Nb_clique, mu_clique(i));
    [rmse, stdd] = RMSEAndSTDForMatrix_bigData(a_greedy, a_skp);
